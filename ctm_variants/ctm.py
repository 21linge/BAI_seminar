...
# this file needs to be moved into continuous_thought_machines/models and replace the normal one.
...
class ContinuousThoughtMachine(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, x, track=False):
        ...
        for stepi in range(self.iterations):
            ...
            if self.ablate_nlms:
                # latest pre activation vector as input if nlm ablation
                activated_state = self.trace_processor(state_trace[:, :, -1])
            else:
